...
if sudo docker version >/dev/null 2>&1; then
  ...
  # -G docker: explicitly chown the socket to the "docker" group so "user"
  # (added to that group at image build time) can access it without sudo.
  sudo dockerd --host=unix:///var/run/docker.sock -G docker &
